...
class Agents(object):
    ...
    def is_agent_elevated(self, session_id):
        ...
        elevated = (
            Session()
            .query(models.Agent.high_integrity)
            .filter(models.Agent.session_id == session_id)
            .scalar()
        )
    ...
    def get_language_version_db(self, session_id):
        ...
        language_version = (
            Session()
            .query(models.Agent.language_version)
            .filter(models.Agent.session_id == session_id)
            .scalar()
        )
    ...
    def get_agent_id_db(self, name):
        ...
        agent = (
            Session().query(models.Agent).filter((models.Agent.name == name)).first()
        )
    ...
    def update_dir_list(self, session_id, response):
        ...
        if session_id in self.agents:
            ...
            if this_directory:
                Session().query(models.AgentFile).filter(
                    and_(
    ...
    def update_agent_sysinfo_db(
        ...
        host = (
            ...
            .filter(
                and_(
                    models.Host.name == hostname, models.Host.internal_ip == internal_ip
                )
            )
            .first()
        )
    ...
    def rename_agent(self, old_name, new_name):
        ...
        if os.path.exists(new_path):
            ...
            agent = (
                Session()
                .query(models.Agent)
                .filter(models.Agent.name == old_name)
                .first()
            )
            agent.name = new_name
        ...
        self.save_agent_log(
            old_name, "[*] Agent renamed from %s to %s" % (old_name, new_name)
        )
    ...
    def add_agent_task_db(
        ...
        if session_id not in self.agents:
            ...
            if session_id:
                ...
                pk = (
                    Session()
                    .query(func.max(models.Tasking.id))
    ...
    def process_agent_packet(self, session_id, response_name, task_id, data):
        ...
        if response_name == "ERROR":
            ...
            tasking = (
                ...
                .filter(
                    and_(
                        models.Tasking.id == task_id, models.Tasking.agent == session_id
                    )
                )
                .first()
            )
